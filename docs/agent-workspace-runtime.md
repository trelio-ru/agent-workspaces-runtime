...
временных Workspace-файлов используют правила платформы: Windows допускает
разный регистр букв в записи одного каталога. Проверка дочернего пути требует
точной границы каталога и не принимает соседний префикс либо другой диск.
...
- `worklog/` – отдельная автоматически собранная запись каждого
  содержательного Run с человекочитаемым автором из trusted Workspace overview.
...
6. Агент завершает работу одной `trelio-workspace finish`: bridge проверяет и
   ...
   детерминированную запись `worklog/` со строкой автора Run и отправляет
   candidate. Имя и username – только человекочитаемая атрибуция; каноническим
   аудитом остаётся server-side initiating member. Повтор `finish` использует
   тот же путь, а manual entry старого клиента не дублируется. Если
